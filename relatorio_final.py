...
def gerar_relatorio(df):
    ...
    L(f"| Melhor localização? | Tabuleiro dos Oliveiras (ROI {tab_roi:.0f}%), Morretes ({mor_roi:.0f}%) |")
    ...
    L(f"| Características-chave? | Superhost ({sh_sign}{sh_rec_diff:.0f}% receita), Guest favorite (rating {fav_r:.2f}) |")
    L("| O que comprar? | Apartamento 1-2q em Tabuleiro dos Oliveiras ou Centro |")
    L("")
    L("---")
    L("")
    ...
    L("")
    L("### 6.1 Cenário A: Máximo ROI (Perfil Conservador)")
    L("")
    tab_ol_roi = bairros_conf.loc['Tabuleiro dos Oliveiras']['roi'] if 'Tabuleiro dos Oliveiras' in bairros_conf.index else 0
    mor_roi = bairros_conf.loc['Morretes']['roi'] if 'Morretes' in bairros_conf.index else 0
    L("| Critério | Especificação |")
    L("|-----------|---------------|")
    L("| Tipo | Apartamento |")
    L("| Quartos | 1-2 quartos |")
    L("| Banheiros | 1 banheiro |")
    L("| Bairro | Tabuleiro dos Oliveiras ou Morretes |")
    L("| Preço alvo | R$ 600k - R$ 900k |")
    L(f"| ROI esperado | {min(tab_ol_roi, mor_roi):.0f}-{max(tab_ol_roi, mor_roi):.0f}% |")
    L(f"| Payback | {df[df['suburb'].isin(['Tabuleiro dos Oliveiras','Morretes']) & df['grupo_quartos'].isin(['1 quarto','2 quartos'])]['payback_anos'].median():.0f}-{df[df['suburb'].isin(['Tabuleiro dos Oliveiras','Morretes']) & df['grupo_quartos'].isin(['1 quarto','2 quartos'])]['payback_anos'].median():.0f} anos |")
    L("| Animais | Permitir |")
    L("")
    L("**Justificativa:** ROI maximizado, entrada mais baixa, payback mais curto.")
    L("Risco menor por menor capital em jogo.")
    L("")

    L("### 6.2 Cenário B: Melhor Equilíbrio (Perfil Recomendado)")
    L("")
    ...
    L("| Bairro | Centro ou Morretes |")
    L("| Preço alvo | R$ 900k - R$ 1.2M |")
    L(f"| ROI esperado | {centro_roi:.0f}-{mor_roi:.0f}% |")
    L(f"| Payback | {centro_pb:.0f}-{df[df['suburb']=='Morretes']['payback_anos'].median():.0f} anos |")
    L("| Animais | Permitir |")
    L("| Superhost | Buscar imóveis geridos por superhosts |")
    L("")
    L("**Justificativa:** Maior liquidez (Centro tem mais oferta/demanda), receita superior")
    L("às opções baratas, rating alto, e maior potencial de valorização.")
    L("")
    ...
    L("")
    L("**Recomendação principal: Cenário B** (Apartamento 2q, Centro/Morretes)")
    L("")
    ...
    L(f"1. **ROI competitivo** (~{df[df['grupo_quartos']=='2 quartos']['roi_liquido'].median():.0f}%) sem exigir entrada muito baixa")
    ...
    L(f"4. **Payback gerenciável** — {pb_2q:.1f} anos")
    ...
    L("|--------|--------|-----|---------|---------|--------------|-------|")
    L(f"| **A: Conservador** | Tab. Oliveiras/Morretes | {tab_ol['roi_liquido'].median():.0f}-{mor['roi_liquido'].median():.0f}% | {tab_ol['payback_anos'].median():.0f}-{mor['payback_anos'].median():.0f}a | R$ {mor['receita_anual'].median()/1000:.0f}-{tab_ol['receita_anual'].median()/1000:.0f}k | R$ 600k-900k | Baixo |")
    L(f"| **B: Equilibrado (Recomendado)** | Centro/Morretes | {cen['roi_liquido'].median():.0f}-{mor['roi_liquido'].median():.0f}% | {cen['payback_anos'].median():.0f}-{mor['payback_anos'].median():.0f}a | R$ {cen['receita_anual'].median()/1000:.0f}k | R$ 900k-1.2M | Médio |")
    L(f"| **C: Agressivo** | Morretes/Centro | {m3q_roi:.0f}-{df[(df['suburb']=='Centro') & (df['grupo_quartos']=='3 quartos')]['roi_liquido'].median():.0f}% | {m3q_pb:.0f}-{df[(df['suburb']=='Centro') & (df['grupo_quartos']=='3 quartos')]['payback_anos'].median():.0f}a | R$ {m3q_rec/1000:.0f}-{df[(df['suburb']=='Centro') & (df['grupo_quartos']=='3 quartos')]['receita_anual'].median()/1000:.0f}k | R$ 1-2.5M | Médio-Alto |")
    L(f"| **D: Centro Compacto** | Centro | {c_1q_roi:.0f}-{c_2q_roi:.0f}% | {c_1q_pb:.0f}-{c_2q_pb:.0f}a | R$ {c1q['receita_anual'].median()/1000:.0f}k | R$ {c_1q_pv/1e6:.1f}M | Baixo-Médio |")
    L("")
    L("---")
    L("")
    ...
    L("")
    L(f"2. **O bairro é o fator #1:** Tabuleiro dos Oliveiras tem ROI de {bairros_conf.loc['Tabuleiro dos Oliveiras']['roi']:.0f}% — maior entre")
    L("   bairros com dados confiáveis. A escolha do bairro impacta mais que qualquer outra variável.")
    L("")
    sh_sign_conc = '+' if sh_preco_diff >= 0 else ''
    L(f"3. **Superhost merece atenção:** Nesta amostra, superhosts cobram {sh_sign_conc}{sh_preco_diff:.0f}% por noite e")
    L(f"   faturam {sh_rec_sign}{sh_rec_diff_val:.0f}% ao ano, mas acumulam +{sh_rev_diff_final:.0f}% mais reviews.")
    L("   O preço menor pode refletir composição da amostra (imóveis menores/menos valorizados).")
    L("   Seazone deveria investigar se o status de superhost é determinante ou se o efeito é de")
    L("   bairro/tamanho. Ação recomendada: buscar imóveis com histórico de superhost como")
    L("   sinal de gestão eficiente, não como garantia de retorno financeiro superior.")
    L("")
    L("4. **Centro é competitivo para 1-2q:** A hipótese interna está parcialmente")
    L("   correta. O diferencial está em combinar 1-2 quartos com perfil de favorito.")
    L("")
    ...
    L("")
    L("6. **Payback realista:** Mesmo no melhor cenário, payback é de 7+ anos.")
    L("   Investimento de longo prazo, não especulativo.")
    L("")
    L("---")
    L("")
